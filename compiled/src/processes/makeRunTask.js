...
function makeRunTask(runners) {
    return function (task) {
        if (!task) {
            return rxjs_1.throwError(new Error('argument.null.task'));
        }
        if (!task.kind) {
            return rxjs_1.throwError(new Error('argument.null.task.kind'));
        }
        var runner = runners[task.kind];
        if (!runner) {
            return rxjs_1.throwError(new Error("unknown.task:" + task.kind));
        }
